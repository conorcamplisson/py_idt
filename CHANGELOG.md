...
Renamed. PyPI rejected `py_idt` as too similar to the existing `idt` project — it strips the `py`
prefix when comparing names. The distribution is now `py-idt-order` and the import is
`py_idt_order`.
...
Neither `0.1.0` nor `0.1.1` was ever published, so nothing depends on the old name.
...
- Packaging brought to the develop-python canon: hatchling replaces setuptools, the version moves
  to `src/py_idt_order/_version.py`, and the package moves to a `src/` layout. No functional change to
  any module.
...
[Unreleased]: https://github.com/conorcamplisson/py_idt/compare/v0.1.2...HEAD
[0.1.2]: https://github.com/conorcamplisson/py_idt/releases/tag/v0.1.2
